...
class FinanceModel {
    ...
    // Get all purchase requests from supply department (updated for new schema)
    static async getAllPurchaseRequests() {
        const SQL_COMMAND = `
            SELECT 
                ...
                DATE_FORMAT(pr.created_date, '%Y-%m-%d %H:%i:%s') as created_date_formatted
            ...
            ORDER BY 
                CASE 
                    ...
                    WHEN pr.status = 'Finance Approved' THEN 2
                    WHEN pr.status = 'Finance Rejected' THEN 3
                    WHEN pr.status = 'Converted to PO' THEN 4
                    ELSE 5
                END,
        ...
        try {
            console.log('Executing SQL query for purchase requests (new schema)...');
    ...
    // List purchases that have supplier-provided delivery cost/discount and are pending finance approval
    static async getPendingPurchaseEstimations() {
        const SQL = `
            SELECT 
                p.purchase_id,
                p.pr_id,
        ...
        try {
            ...
            const [allPurchases] = await db.query('SELECT COUNT(*) as count FROM purchases');
            const [statusCheck] = await db.query('SELECT status, COUNT(*) as count FROM purchases GROUP BY status'); 
            ...
            throw new Error('Failed to fetch pending purchase estimations');
        }
    }

    // Approve a supplier estimation: set purchase status to 'Processed'
    static async approvePurchaseEstimation(purchaseId) {
        try {
            // Compute final total and persist as invoice_amount when approving
            const [rows] = await db.query(`
                SELECT quantity, unit, unit_price, COALESCE(delivery_cost,0) AS delivery_cost, COALESCE(discount,0) AS discount
                FROM purchases WHERE purchase_id = ? AND status = 'Pending'
            `, [purchaseId]);
    ...
    static async updatePurchaseEstimation(purchaseId, status, remarks = null, delivery_cost = null, discount = null) {
        try {
            ...
            if (result.affectedRows === 0) {
                return { 
                    success: false, 
                    message: 'Purchase not found or not in Pending status' 
                };
            }
            ...
            throw new Error(`Failed to ${status.toLowerCase()} purchase estimation: ${error.message}`);
        }
    }

    // Update purchase request status (for finance approval/rejection) - updated for new schema
    static async updatePurchaseRequestStatus(requestId, status, remarks = null) {
        const connection = await db.getConnection();
        try {
            await connection.beginTransaction();

            // If approving, create purchase order
            if (status === 'Finance Approved') {
                // Get the purchase request details
                const [requestRows] = await connection.query(`
                    SELECT 
                        pr_id, supplier_id, material_id, variant, 
                        quantity_requested, unit, unit_price, total_price
                ...
                // Insert into purchases table (Purchase Order for supplier)
                await connection.query(`
                ...
                // For other status updates (rejection, etc.)
    ...
    // Update purchase request status to approved (for finance approval) - updated for new schema
    static async approvePurchaseRequest(requestId) {
        const connection = await db.getConnection();
        try {
            await connection.beginTransaction();

            // First, get the purchase request details
            const [requestRows] = await connection.query(`
                SELECT 
                    pr_id, supplier_id, material_id, variant, 
                    quantity_requested, unit, unit_price, total_price
            ...
            // Update the purchase request status to Finance Approved
            await connection.query(`
                UPDATE purchase_requests 
            ...
            return {
                success: true,
                message: 'Purchase request approved successfully and purchase order created',
    ...
    // Get approved purchase requests (for finance view) - updated for new schema
    static async getApprovedPurchaseRequests() {
        const SQL_COMMAND = `
            ...
            LEFT JOIN materials m ON pr.material_id = m.material_id
            LEFT JOIN users u ON pr.requested_by = u.id
            WHERE pr.status = 'Finance Approved'
    ...
    static async getPurchaseOrdersWithEstimations() {
        ...
        try {
            ...
            console.error('🔍 DEBUG: Error in getPurchaseOrdersWithEstimations:', error);
            throw new Error('Failed to fetch purchases with estimations');
        }
    }

    // Update purchase order status (approve/reject estimation)
    static async updatePurchaseOrderEstimation(poId, status, remarks = null, payment_type = null, delivery_cost = null, discount = null) {
        const SQL_COMMAND = `
            UPDATE purchases 
            SET 
                status = ?,
        ...
        try {
            ...
            if (status === 'Processed' && (delivery_cost !== null || discount !== null)) {
                await db.query(
                    `UPDATE purchases SET 
                        ...
                        discount = ?
                     WHERE purchase_id = ? AND status = 'Pending'`,
                    [delivery_cost || 0, discount || 0, poId]
                );
            }
            ...
            return {
                success: true,
                message: `Purchase ${status.toLowerCase()} successfully${payment_type ? ` with ${payment_type} payment` : ''}`
            };
        } catch (error) {
            console.error('Error in updatePurchaseOrderEstimation:', error);
            throw new Error(`Failed to ${status.toLowerCase()} purchase: ${error.message}`);
        }
    }
    ...
    // =============================================
    // REFUND MANAGEMENT
    // These functions handle refund operations
    // =============================================
    ...
    // Update refund status (Finance approval/rejection)
    static async updateRefundStatus(purchaseId, status, remarks = null) {
        const connection = await db.getConnection();
        try {
            await connection.beginTransaction();

            // Validate the purchase exists and is in Return Pending status
            const [purchaseRows] = await connection.query(`
                SELECT purchase_id, status, supplier_id, total_price, invoice_amount, pr_id
                FROM purchases 
                WHERE purchase_id = ? AND status = 'Return Pending'
            `, [purchaseId]);
            ...
            if (purchaseRows.length === 0) {
                ...
                return { 
                    success: false, 
                    message: 'Refund request not found or not in pending status' 
                };
            }
            ...
            let finalStatus;
            if (status === 'Refunded') {
                finalStatus = 'Cancelled'; // Refunded orders are cancelled
            } else if (status === 'Returned') {
                finalStatus = 'Returned';
            } else if (status === 'Refund Rejected') {
                finalStatus = 'Refund Rejected';
            } else {
                await connection.rollback();
                return { 
                    success: false, 
            ...
            // If refunded (cancelled), also update the linked purchase_request
            if (status === 'Refunded' && purchase.pr_id) {
                await connection.query(`
                    UPDATE purchase_requests 
                ...
                console.log(`Purchase ${purchaseId} and linked purchase_request ${purchase.pr_id} cancelled due to refund, amount: ${purchase.invoice_amount || purchase.total_price}`);
            }
    ...
    static async redeliverOrder(purchaseId) {
        const connection = await db.getConnection();
        try {
            await connection.beginTransaction();

            // Validate the purchase exists and is in Returned status
            const [purchaseRows] = await connection.query(`
                SELECT purchase_id, status, supplier_id, total_price, invoice_amount
                FROM purchases 
                WHERE purchase_id = ? AND status = 'Returned'
            `, [purchaseId]);
            ...
            if (purchaseRows.length === 0) {
                ...
                return { 
                    success: false, 
                    message: 'Returned order not found or not in returned status' 
                };
            }
            ...
            await connection.rollback();
            console.error('Error in redeliverOrder:', error);
            throw error;
        } finally {
            connection.release();
        }
    ...
    static async getAllPurchaseOrders() {
        const sql = `
            SELECT 
                ...
                p.status,
                DATE_FORMAT(p.created_date, '%Y-%m-%d %H:%i:%s') AS created_date,
                DATE_FORMAT(p.invoice_date, '%Y-%m-%d %H:%i:%s') AS invoice_date_formatted
            FROM purchases p
            LEFT JOIN supplier_account sa ON sa.supplier_id = p.supplier_id
            LEFT JOIN materials m ON m.material_id = p.material_id
            ORDER BY p.created_date DESC
